...
def plot_for_two(output_image, target_image, prompt):
    """
    Get plot for output image and target image
    """
...
def log_coefficients_hist(run, model, steps=[0, 9, 24]):
    """
    Histogram for coefficients for logging
    """
...
def calculate_hpsv2_loss(image, text, hpsv2_model):
    """
    Differentiable hpsv2 loss
    """
    image = torchvision.transforms.functional.resize(image, (224, 224))
    
    #трансформ из https://github.com/openai/CLIP/blob/dcba3cb2e2827b402d2701e7e1c7d9fed8a20ef1/clip/clip.py#L79
    image = (image + 1) / 2
    ...
    h_tokenizer = get_tokenizer("ViT-H-14")
    text = h_tokenizer(text).to(device)
    image_features = hpsv2_model.encode_image(image)
    text_features = hpsv2_model.encode_text(text)
    
    image_features = image_features / image_features.norm(dim=1, keepdim=True)
    text_features = text_features / text_features.norm(dim=1, keepdim=True)
    cosine_sim = (image_features @ text_features.T).mean()
    cosine_sim_loss = 1.0 - cosine_sim
...
if __name__=="__main__":
    ...
    best_model_filename = "best_model_hpsv2.pt"
    log_coefficients_hist(run=run, model=model)
    run["description"] = f"25 шагов, 25 * 24 коэффициентов, hpsv2"
    ...
    epoch_start = 0
    hpsv2_model, _, preprocess_val = create_model_and_transforms(
        model_name="ViT-H-14",
        device=device
    )
    for params in hpsv2_model.parameters():
    ...
    for epoch in range(epoch_start, EPOCH):
        ...
        for idx, (x, y, prompt) in enumerate(tqdm(train_loader, desc=f"train, epoch {epoch}")):
            ...
            hpsv2_loss = calculate_hpsv2_loss(output, prompt, hpsv2_model)
            batch_loss = 0.5 * loss_fn_vgg(output.float(), y.float()).mean() + 0.5 * hpsv2_loss
        ...
        if avg_val_loss < best_val_loss:
            ...
            # надежнее, чем просто torch.save
