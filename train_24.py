...
class TrainCoefs(nn.Module):
    ...
    Negative embeddings are similiar for each prompt (empty string)
    """
    def __init__(self, pipe, final_layer_norm) -> None:
        ...
        logits = torch.full((24, ), fill_value=-1e9, dtype=torch.float32)
        logits[-1] = 0.0
        self.coefficients = nn.Parameter(logits)
    ...
    def get_embed_with_coef(self, hidden_states):
        """
        ...
        weighted = hidden_states * torch.nn.functional.softmax(self.coefficients, dim=-1).to(hidden_states.dtype).view(1, 24, 1, 1)
    ...
    def prepare_intermediate_images(self, batch_size, num_channels, height, width, dtype, generator):
        """
        Function for custom pipe (from IFPipeline)
        """
    ...
    def generate_image_from_emb(self, pos_emb, neg_emb):
        ...
        images = self.custom_pipe(prompt_embeds=pos_emb, negative_prompt_embeds=neg_emb)

        return images

    def forward(self, outputs):
        outputs = outputs.to(dtype=torch.float16)
        pos_embedding = self.get_embed_with_coef(outputs)
        neg_embedding = self.get_embed_with_coef(self.neg_embeddings)
        pos_embedding = self.final_layer_norm(pos_embedding)
        neg_embedding = self.final_layer_norm(neg_embedding)
        neg_embedding = neg_embedding.repeat(pos_embedding.shape[0], 1, 1)
...
def log_coefficients_hist(run, model):
    """
    ...
    ax.bar(range(len(coeff_np)), coeff_np)
    ax.set_xlabel("coefficient")
    ax.set_ylabel("after softmax")
    ax.set_ylim(0, max(coeff_np) + 0.03)
    plt.tight_layout()

    run[f"coefficients"].append(fig)
    plt.close(fig)


if __name__=="__main__":
    
    train_dataset = CustomDataset(start_num_samples=0, end_num_samples=80) # Фиксированный трейн между запусками
    val_dataset = CustomDataset(start_num_samples=80, end_num_samples=100) # Фиксированная валидация между запусками
    ...
    best_val_loss = 100
    patience = EPOCH # количество эпох для early stopping
    counter = 0
    best_val_metric = 0
    warmup_steps = 500
    ...
    for epoch in range(epoch_start, EPOCH):
        ...
        for idx, (x, y, prompt) in enumerate(tqdm(train_loader, desc=f"train, epoch {epoch}")):
            ...
            # Сохранение картинок для логирования
            if idx % 100 == 0:
            ...
            # Проверка, что градиенты считаются
        ...
        # Сохранение наилучшей модели по качеству валидации
